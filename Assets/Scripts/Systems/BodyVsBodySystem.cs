...
namespace BB
{
    ...
    public partial struct BodyVsBodySystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            state.Dependency = Physics.FindPairs(in rigidBodyLayer, in findBodyBodyProcessor).ScheduleParallel(state.Dependency);
        }
        ...
        struct FindBodyVsBodyProcessor : IFindPairsProcessor
        {
            ...
            public void Execute(in FindPairsResult result)
            {
                ...
                //if (rigidBodyA.ignoreSimulation && rigidBodyB.ignoreSimulation)
                //    return;
                if (rigidBodyA.ignoreSimulation || rigidBodyB.ignoreSimulation)
                    return;

                if (rigidBodyA.isObstacle && rigidBodyB.isObstacle)
                    return;
